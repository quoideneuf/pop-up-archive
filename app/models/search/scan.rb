...
class Search::Scan
  ...
  def url;                Search::Configuration.url + "/_search/scroll";                           end
  ...
  def __perform
    @response  = Search::Configuration.client.get [url, params].join, scroll_id
  ...
  def __logged(error=nil)
    if Search::Configuration.logger
      Search::Configuration.logger.log_request 'scroll', nil, to_curl

      took = @json['took']        rescue nil
      code = @response.code       rescue nil
      body = "#{@seen}/#{@total} (#{@seen/@total.to_f*100}%)" rescue nil

      Search::Configuration.logger.log_response code || 'N/A', took || 'N/A', body
    end
  end
